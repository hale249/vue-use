...
export function useScriptTag(
  ...
  const loadScript = (waitForScriptLoad: boolean): Promise<HTMLScriptElement | boolean> =>
    new Promise((resolve, reject) => {
      ...
      let el = document.querySelector(`script[src="${src}"][data-script-tag="true"]`) as HTMLScriptElement;
  ...
  const unload = () => {
    ...
    if (scriptTag.value) {
      head ? document.head.removeChild(scriptTag.value) : document.body.removeChild(scriptTag.value);
